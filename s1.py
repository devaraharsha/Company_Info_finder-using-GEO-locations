    ...
    if isinstance(items, list):
        ...
        for i in items:
            if isinstance(i, dict):
                if 'email' in i:
                    cleaned.append(i['email'])
                elif 'number' in i:
                    cleaned.append(i['number'])
                else:
...
def find_manager_via_linkedin(company_name, location_filter=None):
    titles = [
        "Operations Manager",
        "Manager",
        "General Manager",
    ...
    for title in titles:
        try:
            search_query = rr.person.search().filter(
                current_employer=company_name,
                current_title=title
            )
            if location_filter:
                # Only add location if provided and supported
                search_query = search_query.filter(location=location_filter)
            result = search_query.execute()
            for person in result.people:
                name = getattr(person, "name", "N/A")
                linkedin_url = getattr(person, "linkedin_url", "N/A")
                emails = safe_join(getattr(person, "emails", []))
                phones = safe_join(getattr(person, "phones", []))
                return name, linkedin_url, emails, phones
        except Exception as e:
            print(f"RocketReach search error for title '{title}':", e)
    ...
    if lat and lng:
        try:
            ...
            if not place:
                ...
                manager_name, manager_linkedin, email, manager_phone = find_manager_via_linkedin(company_name, location_filter='Hyderabad')
                manager_name = manager_name or 'N/A'
                manager_linkedin = manager_linkedin or 'N/A'
                email = email or 'N/A'
                manager_phone = manager_phone or 'N/A'
                save_to_csv([company_name, maps_link, address, phone_number, website, manager_name, manager_linkedin, email, manager_phone])
                result = {
