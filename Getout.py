...
def find_manager_via_linkedin(company_name):
    linkedin_test_url = "https://www.linkedin.com/in/"  # Change if you have specific URL
    try:
        result = rr.person.lookup(linkedin_url=linkedin_test_url)
        if hasattr(result, "person"):
            p = result.person
            name = getattr(p, "name", "N/A")
            emails = getattr(p, "emails", [])
            emails_str = ", ".join(emails) if emails else "N/A"
            phones = getattr(p, "phones", [])
            phones_str = ", ".join(phones) if phones else "N/A"
            linkedin_url = linkedin_test_url
            return name, linkedin_url, emails_str, phones_str
    except Exception as e:
        print("LinkedIn URL lookup error:", e)
    try:
